...
Public Class UserCarUpdate
    ...
    Public mypubVIN As String

    Private Sub UserCarUpdate_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        mypubVIN = UserCarHome.pubVIN

        sqlConn.ConnectionString = Server
        sqlConn.Open()
        query = "SELECT  FORM_NUM, concat(LAST_NAME,', ',FIRST_NAME) As Name, CERT_EMM_COMPLIANCE AS Q1, OG_SALES_INV AS Q2, 
                CERT_COVER AS Q3, POLICE_CLEARANCE AS Q4, CERT_STOCK AS Q5, PAYMENT_REF_NUM AS Q6, VEH_COLOR, PLATE_NUM 
                from applicant inner join emission inner join form inner join vehicle_info where applicant.APP_CODE=" & str & "
                 and applicant.APP_CODE = vehicle_info.APP_CODE and emission.EMM_CODE=vehicle_info.EMM_CODE and vehicle_info.VIN='" & mypubVIN & "'
                 and emission.EMM_CODE = form.EMM_CODE;"
        ...
        If String.IsNullOrEmpty(TB3.Text) = False Then
            ...
            With sqlCmd
                .CommandText = "Update form, emission, applicant, vehicle_info
	                    ...
	                    WHERE vehicle_info.VIN = '" & mypubVIN & "' and vehicle_info.EMM_CODE=emission.EMM_CODE  AND applicant.APP_CODE=form.APP_CODE and applicant.APP_CODE = vehicle_info.APP_CODE
	                    AND emission.EMM_CODE = form.EMM_CODE;
                ...
                then '1' else '0' end where emission.EMM_CODE=form.EMM_CODE and applicant.APP_CODE= '" & str & "' 
                and form.FORM_NUM=@FORM_NUM;"
                ...
                .Parameters.AddWithValue("@FORM_NUM", FORM_NUM.Text)
